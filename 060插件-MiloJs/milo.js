...
(function () {
    var config = {
        loaderPath: window.useBundleVersion ? location.protocol + "//ossweb-img.qq.com/images/js/milo_bundle/" : location.protocol + "//ossweb-img.qq.com/images/js/milo_bundle/",
        version: "20130701",
        expires: 3e4 // 30000 半小时
    };
...
(function () {
    ...
    extend(string, {
        ...
        cutChinese: function (str, iMaxBytes, sSuffix) {
            if (isNaN(iMaxBytes))
                return str;
            if (string.getByteLength(str) <= iMaxBytes)
                return str;
            var i = 0
                , bytes = 0;
            for (; i < str.length && bytes < iMaxBytes; ++i,
                ++bytes) {
                if (str.charCodeAt(i) > 255) {
                    ++bytes
                }
